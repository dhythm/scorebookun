...
> [!NOTE]
> 現在はα版です。仕様や保存データの形式は予告なく変わることがあります。アプリ内でも名前の横に「α版」と表示しています。
...
- トップページの要望アイコンから、不具合・改善要望の連絡先を表示
...
- 試合の作成時に「削除キー」（任意、4〜100文字）を設定すると、試合履歴の削除ダイアログでそのキーを入力した人だけが、試合をサーバーから削除できます。削除すると全員の画面から消え、元に戻せません。キーはあとから設定・変更・確認できず、未設定の試合はアプリからは削除できません。
- 削除キーはソルト付きのscryptハッシュだけを保存し、試合データや共有URLには含めません。記録中の試合が他の人に削除された場合は、画面上部に通知を表示します（端末の控えは残ります）。
...
トップページのヘッダーにある要望アイコンから連絡先を確認できます。不具合の報告や改善要望は、Xの [@dhythm_dev](https://x.com/dhythm_dev) までお寄せください。公開投稿には個人情報や共有したくない試合データを含めないようご注意ください。
...
| テーブル                 | 内容                                                                                                                                        |
| ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------- |
| `games`                  | 試合ID、楽観ロック用の `version` と `last_mutation_id`、削除キーのハッシュ（`delete_key_hash`、未設定はNULL）、状態、開始日時、規定イニング |
